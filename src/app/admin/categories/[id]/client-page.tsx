...
import { FaArrowLeft, FaImage, FaTimes, FaTrash, FaExclamationTriangle, FaFolder, FaFolderOpen, FaGripVertical } from "react-icons/fa";

type Category = Tables<'categories'>;

export function EditCategoryPage({ id: categoryId }: { id: string }) {
    ...
    // Helpers borrowed from NewCategoryPage (adapted)
    ...
    function selectParentCategory(id: string) {
        // Prevent selecting itself or a descendant
        if (id === categoryId || isAncestorOf(categoryId, id)) {
            return;
        }
    ...
    // New UI parity states (match "new" page UX)
    ...
    async function fetchCategoryData() {
        try {
            ...
            setIsSubcategory(!!data.parent_category_id);
    ...
    async function handleDelete() {
        try {
            ...
            if (childCategories && childCategories.length > 0) {
                throw new Error('Bu kategorinin alt kategorileri var. Önce alt kategorileri silin veya başka bir kategoriye taşıyın.');
            }
    ...
    function buildCategoryOptions(categories: Category[], parentId: string | null = null, level = 0): JSX.Element[] {
        ...
        for (const category of children) {
            if (category.id !== categoryId && !isAncestorOf(categoryId, category.id)) {
                options.push(
    ...
    function isAncestorOf(categoryId: string, potentialDescendantId: string): boolean {
        let current = categories.find(c => c.id === potentialDescendantId);
        while (current) {
    ...
    const currentCategoryIdInPath = path.length > 0 ? path[path.length - 1] : null;
    const subCategories = categories.filter(c => c.parent_category_id === currentCategoryIdInPath);
    const filteredCategories = categories.filter(c =>
        getCategoryPath(c.id).toLowerCase().includes(search.toLowerCase())
    ).filter(c => c.id !== categoryId && !isAncestorOf(categoryId, c.id));
        ...
        return (
            <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex items-center justify-center py-12">
                    ...
                    <span className="ml-3 text-gray-600 dark:text-gray-400">Kategori yükleniyor...</span>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="flex items-center justify-between mb-8">
                <div>
                    <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Kategori Düzenle</h1>
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-6">
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-4">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <label className="relative flex items-center p-4 border-2 border-gray-200 dark:border-gray-600 rounded-lg opacity-60 cursor-not-allowed">
                                        <input
                                            type="radio"
                                            className="sr-only"
                                            name="category_type"
                                        ...
                                        <div className="flex-1">
                                            ...
                                            <div className="text-sm text-gray-500 dark:text-gray-400">Üst seviye kategori</div>
                                        </div>
                                    ...
                                    <label className="relative flex items-center p-4 border-2 border-gray-200 dark:border-gray-600 rounded-lg opacity-60 cursor-not-allowed">
                                        ...
                                        <div className="flex-1">
                                            ...
                                            <div className="text-sm text-gray-500 dark:text-gray-400">Mevcut kategori altında</div>
                                        </div>
                                        <FaFolderOpen className="text-gray-400 ml-2" />
                                    </label>
                                </div>
                                <p className="mt-3 text-xs text-gray-500 dark:text-gray-400">Düzenleme modunda kategori tipi değiştirilemez.</p>
                            </div>
                            ...
                            <div className="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-4 border border-blue-200 dark:border-blue-800">
                                ...
                                <div className="space-y-2">
                                    <div className="flex rounded-lg overflow-hidden border border-blue-300 dark:border-blue-600">
                                        <input
                                            type="text"
                                            value={isSubcategory ? (selectedParentName || "Üst kategori seçin") : "Ana Kategori"}
                                            readOnly
                                            className="flex-1 px-4 py-3 bg-white dark:bg-gray-700 text-gray-900 dark:text-white border-0 focus:ring-0"
                                        />
                                    </div>
                                    <p className="text-xs text-blue-700 dark:text-blue-300">Düzenleme modunda kategori konumu değiştirilemez.</p>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">Düşük sayılar önce görünür</p>
                                </div>
                                ...
                                <div>
                                    ...
                                    <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">Slug düzenleme, düzenleme modunda kapalıdır</p>
                                </div>
                            </div>
                        </div>
                    </div>
                                        ...
                                        <div className="flex items-start space-x-4">
                                            ...
                                            <div className="flex-1 min-w-0">
                                                ...
                                                <p className="text-sm text-gray-500 dark:text-gray-400">
                                                    {imageFile?.name || 'Yüklü görsel'}
                                                </p>
                                                <label htmlFor="image" className="mt-2 cursor-pointer text-sm text-primary hover:text-primary-dark">
                                                    Değiştir
                                                    <input
                                                        id="image"
                                                        name="image"
                    ...
                    <div className="flex justify-between items-center pt-6 border-t border-gray-200 dark:border-gray-700">
                        <button
                            ...
                            className="inline-flex items-center px-4 py-2 border border-red-300 dark:border-red-600 rounded-lg text-red-700 dark:text-red-300 bg-white dark:bg-gray-800 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors"
                        >
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-md w-full">
                        <div className="p-6">
                            ...
                            <div className="mb-6">
                                <p className="text-sm text-gray-500 dark:text-gray-400">
                                    <strong>{formData.name}</strong> kategorisini silmek istediğinizden emin misiniz? 
                                    Bu işlem geri alınamaz ve kategoriye bağlı tüm veriler silinecektir.
                                </p>
                            </div>
            ...
            {/* Parent Picker Modal (disabled in edit) */}
            {false && (
                        ...
                        <div className="flex items-center justify-between p-6 border-b border-gray-200 dark:border-gray-700">
                            <div className="flex items-center space-x-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-gray-500 dark:text-gray-400">Kategorinizin yerleştirileceği üst kategoriyi seçin</p>
                                </div>
                            </div>
                            <button
                                    ...
                                    {currentCategoryIdInPath && (
                                        <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-xl p-4">
                                            <div className="flex items-center justify-between">
                                                ...
                                                <button
                                                    onClick={() => selectParentCategory(currentCategoryIdInPath)}
