...
function LoginContent() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (session) {
        // Store session in all storage mechanisms for better persistence
        try {
          const expiresAt = Math.floor(Date.now() / 1000) + (60 * 60 * 24 * 7); // 7 days

          // Store both in Supabase's expected format and as raw session
          const sessionData = {
            currentSession: session,
            expiresAt: expiresAt,
          };

          // Also store the raw session object directly for better compatibility
          ...
          const rawSessionString = JSON.stringify(rawSessionData);

          console.log('Storing session after login:', session.user.email);

          // Store in localStorage (always, not just for rememberMe)
          localStorage.setItem('supabase.auth.token', sessionString);
          localStorage.setItem('supabase-auth-token', rawSessionString);

          // Store in sessionStorage
          sessionStorage.setItem('supabase.auth.token', sessionString);
          sessionStorage.setItem('supabase-auth-token', rawSessionString);
          sessionStorage.setItem('authSuccess', 'true');

          // Store in cookies for better cross-tab and Vercel compatibility
          const cookieExpiryDate = new Date();
          cookieExpiryDate.setDate(cookieExpiryDate.getDate() + (rememberMe ? 7 : 1)); // 7 days if remember me, 1 day otherwise

          // Get hostname for the cookie domain
          const hostname = window.location.hostname;
          const isLocalhost = hostname === 'localhost' || hostname === '127.0.0.1';

          // Set cookies with appropriate domain settings
          const cookieStr = isLocalhost
            ? `supabase.auth.token=${encodeURIComponent(sessionString)};expires=${cookieExpiryDate.toUTCString()};path=/;SameSite=Lax`
            : `supabase.auth.token=${encodeURIComponent(sessionString)};expires=${cookieExpiryDate.toUTCString()};path=/;SameSite=Lax`;

          document.cookie = cookieStr;
          document.cookie = `sb-auth-token=${encodeURIComponent(sessionString)};expires=${cookieExpiryDate.toUTCString()};path=/;SameSite=Lax`;
          document.cookie = `auth_verified=true;expires=${cookieExpiryDate.toUTCString()};path=/;SameSite=Lax`;
          document.cookie = `sb-access-token=${session.access_token};expires=${cookieExpiryDate.toUTCString()};path=/;SameSite=Lax`;
          document.cookie = `sb-refresh-token=${session.refresh_token};expires=${cookieExpiryDate.toUTCString()};path=/;SameSite=Lax`;

          // Force a direct session update to Supabase client
          await supabase.auth.setSession({
            access_token: session.access_token,
            refresh_token: session.refresh_token
          });

          console.log('Session successfully stored in all mechanisms');
        } catch (storageError) {
