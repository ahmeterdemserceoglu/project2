...
function RegisterConfirmationContent() {
  ...
  const [resendStatus, setResendStatus] = useState<{ loading: boolean; message: string; success: boolean }>({
    loading: false,
    message: '',
    success: false
  });
  ...
  const handleResendVerification = async () => {
    ...
    try {
      ...
      setResendStatus({
        loading: false,
        message: data.success ? 'Doğrulama emaili yeniden gönderildi!' : (data.error || 'Bir hata oluştu.'),
    ...
    <div className="h-screen flex items-center justify-center bg-gray-50 dark:bg-dark">
      <div className="max-w-md w-full p-6 bg-white dark:bg-dark-light rounded-xl shadow-lg">
        <div className="flex flex-col items-center">
          <div className="mb-5 p-3 rounded-full bg-green-100 dark:bg-green-900">
            <svg 
          ...
          <p className="text-gray-600 dark:text-gray-300 text-center mb-6">
            Hesabınız başarıyla oluşturuldu. <span className="font-medium">{email}</span> adresine gönderilen doğrulama linkine tıklayarak hesabınızı aktifleştirebilirsiniz.
          </p>
          <div className="mt-2 border border-gray-200 dark:border-dark-lighter rounded-lg p-4 bg-gray-50 dark:bg-dark-medium w-full">
            <h2 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
              Sonraki Adımlar:
            </h2>
            <ol className="list-decimal text-sm text-gray-600 dark:text-gray-400 pl-5 space-y-1">
              <li>Email kutunuzu kontrol edin</li>
              <li>Onay linkine tıklayın</li>
              <li>Hesabınıza giriş yapın</li>
            </ol>
          </div>
          
          {/* Resend verification section */}
          <div className="w-full mt-6 p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800 rounded-lg">
            <p className="text-sm text-blue-600 dark:text-blue-400 mb-3">
              Email gelmediyse veya süresi geçtiyse, doğrulama emailini yeniden gönderebilirsiniz.
            </p>
            <button
