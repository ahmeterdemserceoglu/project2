...
export default function Register() {
  ...
  const validateForm = () => {
    ...
    if (!formData.terms) {
      const msg = 'Devam etmek için Kullanım Şartları ve Gizlilik Politikası\'nı kabul etmelisiniz.';
      setErrorMessage(msg);
      showNotification(msg, 'error');
      return false;
    }
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        ...
        if (!verificationResponse.ok) {
          ...
          if (responseData.message === 'Email is already verified') {
            showNotification('Email adresiniz zaten doğrulanmış. Giriş yapabilirsiniz.', 'info');
          } else {
      ...
      showNotification('E-posta adresinize doğrulama bağlantısı gönderildi. Lütfen kontrol edin.', 'info');
      ...
      let errorMsg = 'Kayıt işlemi sırasında bir hata oluştu. Lütfen tekrar deneyin.';
      
      if (error.message) {
        if (error.message.includes('already registered')) {
          errorMsg = 'Bu email adresi zaten kayıtlı. Lütfen giriş yapın veya farklı bir email adresi kullanın.';
        } else if (error.message.includes('weak password')) {
          errorMsg = 'Şifre çok zayıf. Lütfen en az 8 karakter, bir büyük harf ve bir rakam içeren bir şifre seçin.';
        } else if (error.message.includes('password')) {
          errorMsg = 'Geçersiz şifre. Şifreniz en az 6 karakterden oluşmalıdır.';
        } else if (error.message.includes('email')) {
          errorMsg = 'Geçersiz email formatı. Lütfen geçerli bir email adresi girin.';
        } else if (error.message.includes('network')) {
          errorMsg = 'Ağ bağlantısı hatası. Lütfen internet bağlantınızı kontrol edin.';
        } else if (error.message.includes('Database error')) {
          errorMsg = 'Veritabanı hatası. Bu email zaten sistemde kayıtlı olabilir veya bir iç sunucu hatası oluşmuş olabilir.';
        } else if (error.code === "unexpected_failure") {
          errorMsg = 'Beklenmeyen bir hata oluştu. Bu email zaten kullanılmış olabilir veya sistemde bir sorun olabilir.';
        } else if (error.status === 422 || error.status === 400) {
          if (error.message.includes('Password')) {
            errorMsg = 'Şifre gereksinimleri karşılanmıyor. Daha güçlü bir şifre belirleyin.';
          } else {
            errorMsg = 'Geçersiz kayıt bilgileri. Lütfen bilgilerinizi kontrol edin.';
          }
        } else if (error.status === 429) {
          errorMsg = 'Çok fazla deneme yapıldı. Lütfen daha sonra tekrar deneyiniz.';
      ...
      <div className="w-full md:w-1/2 flex items-center justify-center p-4 md:p-8 lg:p-12 overflow-y-auto">
        <div className="w-full max-w-md py-2">
          <div className="text-center md:text-left mb-6">
            ...
            <h1 className="text-2xl md:text-3xl font-bold text-gray-800 dark:text-white mb-2">Hesap Oluştur</h1>
          ...
          <form className="space-y-3" onSubmit={handleRegister}>
            ...
            <div className="flex items-start">
              ...
              <label htmlFor="terms" className="ml-2 block text-sm text-gray-700 dark:text-gray-200">
                <Link href="/terms" className="text-secondary hover:text-secondary-dark">Kullanım Şartları</Link>
                {' '}ve{' '}
                <Link href="/privacy" className="text-secondary hover:text-secondary-dark">Gizlilik Politikası</Link>
                'nı kabul ediyorum.
              </label>
            </div>
          ...
          {debugMode && debugInfo && (
            <div className="mt-4 p-3 bg-gray-50 dark:bg-dark-medium rounded-lg text-xs overflow-auto max-h-60">
              <h4 className="font-medium text-sm text-gray-700 dark:text-gray-300 mb-2">Hata Detayları:</h4>
