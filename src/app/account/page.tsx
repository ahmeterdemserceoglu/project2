  ...
  const loadUserProfile = async () => {
    try {
      ...
      if (profileError) {
        ...
        // We can create a profile or handle as appropriate.
        // For now, assume profile should exist or error is critical.
        console.warn('Profile not found for user, might be a new registration:', profileError.message);
        // Create a default profile structure from user data if needed
        ...
        // Optionally, you might want to insert this default profile into the 'profiles' table here.
        // For this example, we'll just use it for display.
        setIsLoading(false);
      ...
      if (!updatedProfileData.first_name && user.user_metadata?.first_name) {
        updatedProfileData.first_name = user.user_metadata.first_name;
        shouldUpdateDb = true;
      }
      if (!updatedProfileData.last_name && user.user_metadata?.last_name) {
      ...
      if (shouldUpdateDb) {
        const { error: updateError } = await supabase
          ...
          .eq('id', user.id);
        if (updateError) {
          console.error('Error updating profile from metadata:', updateError.message);
